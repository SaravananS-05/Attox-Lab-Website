...
const uint8_t dac = 25; // DAC pin for Vref to drive LMP91000. Pin 25 on Attox lab ESP32Stat Rev 3.5
const uint8_t MENB = 5; // LMP91000 enable pin. Hard wired to ground in Attox lab ESP32Stat Rev 3.5
//analog input pins to read voltages
const uint8_t LMP_C1 = 27; // Note C1, C2 not wired to microcontroller in Attox lab ESP32Stat Rev 3.5
const uint8_t LMP_C2 = 39; // Note C1, C2 not wired in microcontroller in Attox lab ESP32Stat Rev 3.5
const uint8_t LMP = 35;    // ADC pin for Vout reading. Pin 35 on Attox lab ESP32Stat Rev 3.5
...
// For Attox lab ESP32Stat Rev 3.5, LED "blinky" pin.
int LEDPIN = 26;
...
inline float biasAndSample(int16_t voltage, uint32_t rate)
{
  ...
  // hard wired in Attox lab ESP32Stat Rev 3.5 to LMP i.e ESP32 pin 32 (ADC1_CH7)
  ...
  // V2 is not measured in  Attox lab ESP32Stat Rev 3.5 and assumed to be half dacVout, calibration helps this see above
  float current = 0;
  ...
  if (LMPgainGLOBAL == 0)                              // using external feedback resistor, hard coded to 1e6 for now with Attox lab ESP32Stat Rev 3.5
...
void writeVoltsCurrentArraytoFile()
{
  ...
  if (file.println("Attox lab Micro+ Rev 3.5 Sweep"))
  {
...
void handleGetSavSecreteJsonNoReboot(AsyncWebServerRequest *request)
{
  ...
  request->send(200, "text/html", "  <head> <meta http-equiv=\"refresh\" content=\"2; URL=wifi.html\" /> <meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"> </head> <body> <h1> Credentials stored to flash on Micro+. </h1>  </body>");
  restartSystem = millis();
}
...
void runWifiPortal()
{
  ...
  if (!MDNS.begin("Micro+")) // see https://randomnerdtutorials.com/esp32-access-point-ap-web-server/
  {
...
void testLMP91000(uint32_t delayTime_ms, uint8_t bias_setting_local)
{
  ...
  // P  made Attox lab a breakout board of LMP91000 chips.
...
// configures server
{
  ...
  server.on("/rebootMicro+", HTTP_GET, [](AsyncWebServerRequest *request)
            {
              // reboot the ESP32
...
void setup()
{
  ...
  Serial.println("Welcome to Micro+, Firmware Rev. 0.1.2!");
  ...
  if (!m_autoconnected_attempt_succeeded)
  {
    ...
    // MDNS.begin("Micro+"); // see https://randomnerdtutorials.com/esp32-access-point-ap-web-server/
  }
